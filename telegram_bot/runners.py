...
from telegram_bot.utils.loggers import MultilineLogger

if TYPE_CHECKING:
    from telegram_bot.config import Config


async def polling_startup(bots: list[Bot], config: Config) -> None:
    for bot in bots:
...
async def webhook_startup(
    ...
    if await bot.set_webhook(
        ...
        return loggers.webhook.info(
            "Main bot webhook successfully set on url '%s'",
            url,
        )
...
async def webhook_shutdown(bot: Bot, config: Config) -> None:
